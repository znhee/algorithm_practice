...
public class Main7576 {
	...
	public static void main(String[] args) throws IOException {
		...
		graph = new int[MAX][MAX];	// 토마토의 상태
		days = new int[MAX][MAX];	// 익는데 걸리는 날짜 수(days)
		que = new LinkedList<>();
		...
		for(int i=0; i<N; i++) {
			...
			for(int j=0; j<M; j++) {
				...
				if(graph[i][j] == 1) que.offer(new int[] {i, j});
			}
		}
		bfs();
	...
	private static void bfs() {
		while(!que.isEmpty()) {
			...
			for(int i=0; i<4; i++) {
				...
				if(graph[nextX][nextY] != 0 || days[nextX][nextY] >= 1) continue;
