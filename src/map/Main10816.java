...
public class Main10816 {
	...
	public static void main(String[] args) throws IOException{
		...
		HashMap<Integer, Integer> ownCard = new HashMap<Integer, Integer>();
		for(int v: card) {
			if(ownCard.containsKey(v))
				ownCard.put(v, ownCard.get(v)+1);
			else
				ownCard.put(v, 1);
		}
		...
		for(int i=0; i<M; i++) {
			...
			if(ownCard.containsKey(tmp)) 
				sb.append(ownCard.get(tmp)).append(" ");
			else 
				sb.append("0 ");
		}
